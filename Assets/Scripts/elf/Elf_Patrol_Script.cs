...
public class Elf_Patrol_Script : MonoBehaviour
{
    bool patrolLeader = false;
    List<GameObject> aurarium = new List<GameObject>();
    Elf_Movement_AI movement;
    ...
    void Start()
    {
        ...
        foreach (var patrol in gm.SleemasiMiner)
        {
        ...
        movement = this.gameObject.GetComponent<Elf_Movement_AI>();
        List<GameObject> test = new List<GameObject>(GameObject.FindGameObjectsWithTag("Elf Patrol"));
    ...
    List<GameObject> getZuraruim(List<GameObject> aur)
    {
        ...
        {
            if (res.GetComponent<ResourceType>().resource == ResourceTypes.Aurarium)
            {
                list.Add(res);
            }
        }
