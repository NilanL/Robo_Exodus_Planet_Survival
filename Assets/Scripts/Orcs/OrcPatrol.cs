...
public class OrcPatrol : MonoBehaviour
{
    ...
    void Start()
    {
        if (this.gameObject.GetComponent<Unit_Name>().unit_Name == Unit_Names.Graxxian_Miner)
            patrolLeader = true;
        ...
        movement = this.gameObject.GetComponent<Orc_Movement_AI>();
        List<GameObject> test = new List<GameObject>(GameObject.FindGameObjectsWithTag("Graxian Patrol"));
    ...
    List<GameObject> getZuraruim(List<GameObject> aur)
    {
        ...
        {
            if (res.GetComponent<ResourceType>().resource == ResourceTypes.Bloodstone)
            {
                list.Add(res);
            }
        }
